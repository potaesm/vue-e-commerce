<template>
  <div class="center">
    <div>
      ...
      <button @click="CreateNew ? (CreateNew = !CreateNew) : (CreateNew = !CreateNew)" class="btn btn-success form-group bn">New Product</button>
        ...
        <div v-for="data in NewProductsList" :key="data.id">
          ...
          <div class="card d" v-if="!data.isEdit">
            ...
            <ul class="list-group list-group-flush">
              ...
              <li v class="list-group-item danger">Category : {{ GetCatById(data.CatID) }}</li>
            </ul>
            <div class="card-body">
...
export default {
  ...
  methods: {
    ...mapActions(['GetProducts', 'ADD_NEW_PRODUCT', 'EditOneProd', 'DeleteOneProd', 'GetCategories']),
    ...
    saveData(e, ProdID, newData) {
      e.preventDefault();
      let data = { ID: ProdID, name: newData.name, CatID: newData.CatID, price: newData.price, desc: newData.desc, imgSrc: newData.imgSrc };
    ...
    AddNewProduct(e) {
      e.preventDefault();
      if (!this.NewProdData.name || !this.NewProdData.desc || !this.NewProdData.price || !this.NewProdData.imgSrc || !this.NewProdData.CatID) {
